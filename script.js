...
/* ==========================================
   MICRO TOGGLE SIZE BUMP (APPEND ONLY)
   +1px width +1px height (barely noticeable)
   does NOT affect border thickness
   ========================================== */
(() => {
  function setImp(el, prop, value) {
    if (!el) return;
  ...
  function microBumpToggle() {
    const themeBtn = document.getElementById("themeToggle");
    if (!themeBtn) return;

    /* increase size just a hair */
    setImp(themeBtn, "width", "47px");      // was ~46px
    setImp(themeBtn, "min-width", "47px");
    setImp(themeBtn, "max-width", "47px");

    setImp(themeBtn, "height", "35px");     // was ~34px
    setImp(themeBtn, "min-height", "35px");
  ...
  function initMicroBump() {
    microBumpToggle();

    setTimeout(microBumpToggle, 0);
    setTimeout(microBumpToggle, 80);

    window.addEventListener("load", microBumpToggle);
    window.addEventListener("resize", microBumpToggle);
    window.addEventListener("orientationchange", microBumpToggle);
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initMicroBump);
  } else {
    initMicroBump();
  }
})();
